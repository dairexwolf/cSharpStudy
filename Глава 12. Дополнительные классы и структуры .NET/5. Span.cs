...
// Простейшее создание Span:

string[] people = new string[] {"Van", "Billy", "Boy"};
...
Span<string> people2Span = people;

// Перебор Span
foreach (int i in peopleSpan)
	Console.WriteLine(i);
// Далее мы можем получать, устанавливать или перебирать данные также, как в массиве:
people.Span[2] = "Nicolas Kage";	// переустановка значения элемента
...
int[] firstDec = new int[10];		// выделяем память для первой декады
int[] lastDec = new int[10];		// выделяем память для второй декады
Array.Copy(temps, 0, firstDec, 0, 10);		// копируем данные в первый массив
Array.Copy(temps, 20, lastDec, 0, 10);		// копируем данные во второй массив
...
Span<int> tempsSpan = temps;
Span<int> firstDec = tempsSpan.Slice(0, 10);	// нет выделения памяти под данные
Span<int> lastDec = tempsSpan.Slice(20,10);		// нет выделения памяти под данные
...
tempsSpan[0] = 25;					// меняем в temperatureSpan
Console.WriteLine(firstDec[0]);		// 25
...
Console.WriteLine(worldSpan[0]);	// вводим первый символ
