...
string[] teams = { "Бавария", "Боруссия", "Реал Мид", "Манчестер Сити", "ПСЖ", "Барселона", "Крутые перцы" };
...
// Все тоже самое с LINQ
using System.Linq;										// подключаем библиотеку
...
// Итак, что делает этот запрос LINQ? 

// Выражение from t in teams проходит по всем элементам массива teams и определяет каждый элемент как t. Используя переменную t мы можем проводить над ней разные операции.
...
int nums = (from t in teams where t.ToUpper().StartsWith("Б") select t).Count();
